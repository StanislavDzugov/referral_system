...
def register_view(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            user.save()
            return redirect('confirmation', pk=user.pk)
        else:
            if UserProfile.objects.filter(user__phone_number=request.POST.get('phone_number')).exists():
                user = UserProfile.objects.get(user__phone_number=request.POST.get('phone_number'))
                return redirect('confirmation', pk=user.user.pk)
            else:
...
def phone_confirmation_view(request, pk):
...
def profile_view(request):
    ...
    if request.method == 'POST':
        ...
        if UserProfile.objects.filter(user_referral_code=referral_code)\
                .exclude(Q(user__id=request.user.id) | Q(user__is_active=False))\
                .exists():
    ...
    referral_users = UserProfile.objects\
        .filter(activated_referral_code=user_profile.user_referral_code, user__is_active=True)
